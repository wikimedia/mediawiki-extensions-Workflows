( function ( mw, $ ) {
	...
	workflows.ui.dialog.WorkflowOverview.prototype.initialize = function () {
		...
		mw.loader.using( 'ext.workflows.dialog.overview', () => {
	...
	workflows.ui.dialog.WorkflowOverview.prototype.switchPanel = function ( name, data ) {
		...
		switch ( name ) {
			case 'list':
				...
				this.mode = 'mode';
				abilities = { 'abort-arm': false, abort: false, restore: false, 'restore-arm': false, cancel: true, back: false };
			...
			case 'details':
				...
				this.setSize( 'large' );
				mode = this.workflow.getState();
				if ( mode !== 'running' && mode !== 'aborted' ) {
					mode = 'inactive';
				}
				this.actions.setMode( mode );
				abilities = { 'abort-arm': false, abort: false, restore: false, 'restore-arm': false, cancel: true, back: true };
				if ( mode === 'running' ) {
