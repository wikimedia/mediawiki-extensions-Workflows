...
workflows.editor.inspector.Inspector.prototype.getForm = function () {
	return new mw.ext.forms.standalone.Form( {
		...
		data: this.convertDataForForm( Object.assign( {}, this.getElementData() ) )
	} );
};
	...
	return [
		{
			...
			listeners: {
				change: function ( val ) {
					if ( val !== '' ) {
						let id = val.replaceAll( /\W/g, '' ); // eslint-disable-line es-x/no-string-prototype-replaceall
						...
						const elements = workflows.editor.modeler.get( 'elementRegistry' )._elements;
...
workflows.editor.inspector.Inspector.prototype.getExtensionElements = function () {
	const extensionElements = workflows.editor.util.extensionElements.getAll( this.element );
...
workflows.editor.inspector.Inspector.prototype.convertExtensionElements = function ( extensionElements ) { // eslint-disable-line no-unused-vars
	// STUB => OVERRIDE
	return [];
};
...
workflows.editor.inspector.Inspector.prototype.getProperties = function () {
	const formatted = {};
	const properties = this.element.businessObject.get( 'properties' ) || [];
...
workflows.editor.inspector.Inspector.prototype.updateExtensionElements = function ( data ) {
	const ee = workflows.editor.util.extensionElements.getAll( this.element );
	if ( ee ) {
		const toRemove = [];
		for ( let i = 0; i < ee.values.length; i++ ) {
...
workflows.editor.inspector.Inspector.prototype.updateProperties = function ( data ) {
	const oldProp = this.getPropertiesKeyValue();

	for ( const oldPropKey in oldProp ) {
