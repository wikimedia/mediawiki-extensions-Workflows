...
workflows.ui.widget.GroupThresholdValue.prototype.updateHintLabel = function ( data ) {
	...
	if ( data.unit === 'percent' ) {
		let absNumber = Math.floor( data.userCount * ( data.value / 100 ) );
		if ( absNumber === 0 ) {
			absNumber = 1;
		}
		if ( data.userCount === 0 ) {
...
workflows.ui.widget.GroupThresholdValue.prototype.getValue = function () {
	const value = workflows.ui.widget.GroupThresholdValue.parent.prototype.getValue.call( this );
...
workflows.ui.widget.GroupThresholdValue.prototype.updateGroupHint = function () {
	...
	this.timer = setTimeout( () => {
		mws.commonwebapis.group.getByGroupName( this.group ).done( ( data ) => {
			if ( data.length === 0 ) {
				this.removeHint();
				return;
			}
